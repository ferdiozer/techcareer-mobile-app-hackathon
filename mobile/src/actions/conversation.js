...
function setOrderUniqueItems(data) {
  data = _.uniqBy(data, function (e) {
    return e._id;
  });
  //  console.log("!!!!setOrderUniqueItems", { data, sifir: data[0] })
...
export const getConversations = ({ pageNumber = 1 }) => async (
  dispatch,
  getState,
) => {
  return new Promise(async (resolve, reject) => {

    const url = API_ENDPOINTS.getConversations
...
//sendMessage
export const resetGetMessagesByUserId = () => async (dispatch) => {
  dispatch({
...
//addMessage Item
export const addMessageItem = (messageItem) => async (dispatch, getState) => {
  const { messageList } = getState().conversation;
  const newMessageList = [messageItem, ...messageList]
  dispatch({
    type: SEND_MESSAGE_SUCCESS,
    payload: setOrderUniqueItems(newMessageList),
  });
}
...
// Load messages
export const getMessagesByUserId = ({ userId, limit, offset }) => async (
  dispatch,
  getState,
) => {
  ...
  try {
    ...
    const url = API_ENDPOINTS.getMessagesByUserId(userId, limit, offset)
    //  const url = `${API_URL}/me/conversation/messages/${userId}?limit=${limit}&offset=${offset}`
    //  const url = `${API_URL}/me/conversation/messages/${userId}`
    const headers = await myHeaderConfig();
    const serverData = (await axios.get(url, headers)).data
    const { messageList } = getState().conversation;
    const newMessageList = [...serverData, ...messageList]
    // console.log("getMessagesByUserId:action", { serverData })
    dispatch({
      type: GET_MESSAGES_SUCCESS,
      payload: setOrderUniqueItems(newMessageList),
    });
...
export const sendMessage1 = ({
  ...
  getState
    ...
    try {
      ...
      const url = API_ENDPOINTS.createMessage;
      const headers = await myHeaderConfig();
      const serverData = (await axios.post(url, toServerData, headers)).data
      // console.log("!!!!!sendMessage:action:", serverData)
      const { messageList } = getState().conversation;
      // serverData.user = user;
      const newMessageList = [serverData, ...messageList]
      //  console.log("newMessageList:action", { messageList, newMessageList })
      ...
      // //lokal mesajlama ekleme yap
      // //messageList
      // const { messageList } = getState().conversation;
      // dispatch({
      //   type: ADD_MESSAGE,
      //   payload: messageList.push(serverData),
      // });
    } catch (error) {
...
export const addOrUpdateActiveContacts = ({ contacts }) => async (dispatch, getState) => {
  ...
  Object.keys(contacts).forEach((contact) => {
    let conversations = _.filter(data.payload, { meta: { sender: { id: parseInt(contact) } } });
