...
export const saveDeviceDetails = ({ token }) => async (dispatch) => {
  ...
  try {
    ...
    const url = API_ENDPOINTS.setNotificationSubscriptions
    const headers = await myHeaderConfig()
    const serverData = await axios.post(url, pushData, headers)
    console.log("action :saveDeviceDetails ", serverData.data)
    dispatch({ type: SET_PUSH_TOKEN, payload: fcmToken });
  } catch (err) {
    console.log("action :saveDeviceDetails :error", err)
  }
};
